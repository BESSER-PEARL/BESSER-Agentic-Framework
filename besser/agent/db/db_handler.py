...
class DBHandler:
    ...
    def _validate_db_config(self, db_name: str, db_config: dict[str, Any]) -> None:
        missing = sorted(field for field in self._REQUIRED_DB_FIELDS if db_config.get(field) is None)
        if missing:
            raise ValueError(
                f"Missing required DB properties for '{db_name}': {', '.join(missing)}. "
                f"Expected under 'db.sql.{db_name}'."
            )

    def _build_db_url(self, db_name: str) -> URL | str:
    ...
    def connect_to_db(self, db_name: str) -> Connection:
        ...
        if db_name not in self._db_configs:
            raise ValueError(
                f"Unknown database '{db_name}'. Available databases: {', '.join(self.db_names) or 'none'}"
            )

        db_config = self._db_configs[db_name]
        self._validate_db_config(db_name, db_config)
    ...
    def query(self, db_name: str, query: str, llm: 'LLM | None' = None) -> Any:
        ...
        if llm is not None:
            sql_query = self._nl_to_sql(db_name=db_name, nl_query=query, llm=llm)
    ...
    def _nl_to_sql(self, db_name: str, nl_query: str, llm: 'LLM') -> str:
        ...
        if llm_response is None:
            raise ValueError('LLM returned an empty response when translating natural language to SQL.')
