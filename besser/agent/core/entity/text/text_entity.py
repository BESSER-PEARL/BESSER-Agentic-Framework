...
from besser.agent.core.entity.text.text_entity_entry import TextEntityEntry
...
class TextEntity(Entity):
    """The Text Entity core component of an agent.
    ...
    Attributes:
        name (str): The entity's name
        base_entity (bool): Whether the entity is base or not (i.e. custom)
        entries (list[TextEntityEntry] or None): The entity entries. If base_entity, there are no entries (i.e. None)
        description (str or None): a description of the entity, optional
    """
    ...
    def __init__(
        ...
        self.entries: list[TextEntityEntry] or None = None
        if not self.base_entity:
            self.entries = []
            for value, synonyms in entries.items():
                self.entries.append(TextEntityEntry(value, synonyms))
