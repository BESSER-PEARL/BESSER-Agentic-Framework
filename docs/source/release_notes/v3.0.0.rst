...
- Added the :class:`~baf.core.transition.event.Event` and :class:`~baf.core.transition.condition.Condition` classes.
...
- New transition syntax with :class:`~baf.core.transition.transition_builder.TransitionBuilder` class

Improvements
------------

- For each session, transitions from the current state are periodically evaluated for satisfiability (every N seconds, N being configurable with :class:`~baf.CHECK_TRANSITIONS_DELAY` property)
