...
All platforms implement the abstract class :class:`~baf.platforms.platform.Platform`. It defines the methods any
...
- :meth:`~baf.platforms.platform.Platform.initialize`: Initialize the platform. It is called internally when
  you run the agent (:meth:`Agent.run() <baf.core.agent.Agent.run>`). You may need to set some things previously to the
  platform execution.

- :meth:`~baf.platforms.platform.Platform.start`: Start the platform execution. It is called internally when
  you run the agent (:meth:`Agent.run() <baf.core.agent.Agent.run>`).

- :meth:`~baf.platforms.platform.Platform.stop`: Stop the platform. It is called internally when
  you stop the agent (:meth:`Agent.stop() <baf.core.agent.Agent.stop>`).

- :meth:`~baf.platforms.platform.Platform._send`: Send a payload (usually a message) to the user. This is a
  private method. It is called internally by other platform methods, such as the following one.

- :meth:`~baf.platforms.platform.Platform.reply`: Send a textual message to the user.
